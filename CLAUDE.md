...
- Page : héro actif net, barre de progression du capital, grille de stats (CRD, LTV, fonds propres, part apport, intérêts et assurance payés, mensualité, échéances restantes), prochain prélèvement, graphe `AreaChart` sur les 300 échéances (actif net violet + CRD gris, `ReferenceLine` sur l'échéance courante, tooltip `RealEstateTooltip`), bloc « Mon bien » → modal `realestate` (12 champs, `form`), enregistré par `handleRealEstateSave` (édition en place). Sans bien : état vide + bouton Configurer pré-rempli avec `REALESTATE_DEFAULTS`.
