...
| `savingsAccounts` / `savings_accounts` | compte simple `{ id, name, balance }`, portefeuille `{ id, name, isPortfolio:true, holdings:[{ fundId, shares, lastVL, vlAt }], cash }`, **crypto** `{ id, kind:'crypto', sym, qty, lastPrice, priceAt }` (page Crypto), **ou bien immobilier** `{ id, kind:'realestate', name, value, apport, releaseFees, rent, ownerCosts, loan:{ principal, rate, payment, insurance, deferred, months, firstDue, iraFreeAfter } }` (page Immobilier, un seul). Crypto et immobilier sont **exclus** de la page/total Épargne et du sélecteur *Compte Cible* via `isMoneyAccount` ; `reorderSavings` réinjecte le bien immobilier après un réordonnancement |
...
- **Point d'équilibre achat / location** (« règle des 7 ans ») : au bout de combien de temps de détention l'achat bat la location. C'est une **durée**, pas un prix de vente. `avantage(n)` = (valeur − CRD − IRA − mainlevée) + loyers évités − apport − intérêts − assurance − charges de propriétaire − capital emprunté. `breakEven` = 1re échéance où `avantage ≥ 0` (`null` = jamais), `advantage` = avantage à ce jour. **Le capital remboursé n'est pas un coût** : c'est de l'épargne, il revient dans le prix de revente et s'annule des deux côtés.
  - `item.rent` = **loyer de marché** du même logement (défaut 1 000 €/mois). **Surtout pas la mensualité** : une partie est du capital, donc de l'épargne. Avec `rent = payment + insurance`, le calcul dégénère (l'avantage devient l'actif net moins l'apport) et sort un équilibre absurde de 2 ans.
  - `item.ownerCosts` = charges payées par le propriétaire et pas par un locataire, en €/an (taxe foncière + charges de copropriété non récupérables + provision travaux, défaut 2 400 €). Sans elles le calcul est trop favorable à l'achat.
  - Les deux ont un **défaut appliqué quand le champ est absent** (`item.rent ?? …`), pour que les biens enregistrés avant l'ajout affichent la bonne lecture.
  - Avec les défauts : équilibre à **3 ans 9 mois** (nov. 2025), avantage +4 813 € au 15/09/2026. Plus rapide que les 7 ans du dicton pour deux raisons réelles : taux à 1,20 % (contre 3-4 % dans le cas type) et frais de notaire VEFA à ~4 % (contre 7-8 % dans l'ancien).
...
- Page : héro actif net, barre de progression du capital, grille de stats (CRD, LTV, fonds propres, part apport, intérêts et assurance payés, mensualité, échéances restantes), prochain prélèvement, carte « Achat rentable après » (durée, avantage à ce jour, hypothèses rappelées), graphe `AreaChart` sur les 300 échéances (actif net violet + CRD gris + avantage vs location émeraude, `ReferenceLine` horizontale à 0 et verticale « rentable » au point d'équilibre, `ReferenceLine` sur l'échéance courante, tooltip `RealEstateTooltip`), bloc « Mon bien » → modal `realestate` (14 champs, `form`), enregistré par `handleRealEstateSave` (édition en place). Sans bien : état vide + bouton Configurer pré-rempli avec `REALESTATE_DEFAULTS`.
