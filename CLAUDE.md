...
- Sur la page **Épargne**, une section « Patrimoine immobilier » (violet) s'affiche **tout en bas de la liste des comptes, avant les avances** : ligne « Actif net immobilier » (clic → onglet Immobilier) + sous-total **Patrimoine total** = `savingsTotal + cryptoTotal + netAsset`. La carte héro « Épargne Totale » reste `savingsTotal + cryptoTotal` : c'est la « vraie » épargne disponible sur les comptes, l'immobilier n'y entre pas.
