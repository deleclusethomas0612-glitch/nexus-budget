...
- **Seuil de rentabilité** (`threshold`, `gain`, `breakEven`, `lastProfitable`, `missing`) : prix de vente minimum pour ne rien perdre = `principal + apport + intérêts payés + assurance payée + IRA + releaseFees`. **Le capital remboursé n'y figure pas** : ce n'est pas une perte mais de l'épargne, et il s'annule entre le net vendeur et l'argent investi. Évalué **au prix de vente saisi**, sans hypothèse de revalorisation. Le seuil **monte** d'environ 326 €/mois aujourd'hui (intérêts + assurance − baisse de l'IRA), avec une marche à la baisse à l'échéance 181 (fin de l'IRA) : à valeur figée, l'équilibre s'éloigne avec le temps. À 300 000 € il n'est **jamais atteint** (il manque 39 187 € au 15/09/2026) ; il faudrait vendre ~339 200 €.
...
- Page : héro actif net, barre de progression du capital, grille de stats (CRD, LTV, fonds propres, part apport, intérêts et assurance payés, mensualité, échéances restantes), prochain prélèvement, carte « Seuil de rentabilité » (seuil, écart, verdict en clair), graphe `AreaChart` sur les 300 échéances (actif net violet + CRD gris + seuil de rentabilité émeraude, `ReferenceLine` horizontale sur la valeur du bien — son croisement avec la courbe émeraude = le point d'équilibre, `ReferenceLine` sur l'échéance courante, tooltip `RealEstateTooltip`), bloc « Mon bien » → modal `realestate` (12 champs, `form`), enregistré par `handleRealEstateSave` (édition en place). Sans bien : état vide + bouton Configurer pré-rempli avec `REALESTATE_DEFAULTS`.
