...
- **Seuil de rentabilité** : prix de vente minimum pour ne rien perdre, évalué **au prix de vente saisi**, sans hypothèse de revalorisation. Deux lectures, toutes deux dans `realEstateStats` :
  - **brute** (`threshold`, `gain`, `missing`, `since`) = `principal + apport + intérêts payés + assurance payée + IRA + releaseFees`. **Le capital remboursé n'y figure pas** : ce n'est pas une perte mais de l'épargne, et il s'annule entre le net vendeur et l'argent investi. Ce seuil **monte** (~326 €/mois) : à valeur figée l'équilibre s'éloigne, et à 300 000 € il n'est **jamais atteint** (il manque 39 187 € au 15/09/2026).
  - **nette du loyer** (`thresholdNet`, `gainNet`, `missingNet`, `sinceNet`) = la même chose moins `item.rent × n`, les loyers qu'on n'a pas payés en étant propriétaire. C'est la comparaison **acheter plutôt que louer**, la seule qui ait un sens puisque se loger coûte de toute façon. Ce seuil **descend** : à 1 247,85 €/mois (défaut = mensualité assurance comprise) l'équilibre est franchi en **janvier 2024** (échéance 23) et l'écart vaut +29 445 € au 15/09/2026.
  - `item.rent` = loyer de référence mensuel, éditable ; **0 désactive** la lecture nette (la carte et le graphe retombent sur la lecture brute). `since` / `sinceNet` = 1re échéance à l'équilibre (`null` = jamais), `rentAvoided` = loyers évités à ce jour.
...
- Page : héro actif net, barre de progression du capital, grille de stats (CRD, LTV, fonds propres, part apport, intérêts et assurance payés, mensualité, échéances restantes), prochain prélèvement, carte « Seuil de rentabilité » (seuil, écart, verdict en clair), graphe `AreaChart` sur les 300 échéances (actif net violet + CRD gris + seuil de rentabilité émeraude — `seuilNet` si un loyer est saisi, sinon `seuil`, `ReferenceLine` horizontale sur la valeur du bien — son croisement avec la courbe émeraude = le point d'équilibre, `ReferenceLine` sur l'échéance courante, tooltip `RealEstateTooltip`), bloc « Mon bien » → modal `realestate` (13 champs, `form`), enregistré par `handleRealEstateSave` (édition en place). Sans bien : état vide + bouton Configurer pré-rempli avec `REALESTATE_DEFAULTS`.
